...
//            E5j1j2s(L, 0), E5j1k1s(L, 0), E5j2k2s(L, 0);
    ...
    for (int i = 0; i < L; i++) {
        ...
        doublecomplex dE1j1 = 0;
        doublecomplex dE1j2 = 0;
...
//                    if(true) {
                    if (n != m-1) {
//                        doublecomplex asd = -J[j1] * expth * g(n, m) * (eps(dU, i, j1, n, m) / (eps(U0, n, m)))
//                            * ~f[i][n+1] * ~f[j1][m-1] * f[i][n] * f[j1][m];
//                        doublecomplex zxc = -J[i] * expmth * g(n, m) * (eps(dU, i, j2, n, m) / (eps(U0, n, m)))
//                            * ~f[i][n+1] * ~f[j2][m-1] * f[i][n] * f[j2][m];
//                        cout << asd << "\t" << zxc << endl;
//                        cout << i << "\t" << j1 << "\t" << j2 << endl;
//                        double wer = eps(U0, n, m);
//                        double sdf = eps(dU, i, j1, n, m);
//                        double xcv = eps(dU, i, j2, n, m);
//                        cout << wer << "\t" << sdf << "\t" << xcv << endl;
                    dE1j1 += J[j1] * expth * g(n, m) * (eps(dU, i, j1, n, m) / (eps(U0, n, m)))
                            * ~f[i][n+1] * ~f[j1][m-1] * f[i][n] * f[j1][m];
                    dE1j2 += J[i] * expmth * g(n, m) * (eps(dU, i, j2, n, m) / (eps(U0, n, m)))
                            * ~f[i][n+1] * ~f[j2][m-1] * f[i][n] * f[j2][m];
                    }
                }
